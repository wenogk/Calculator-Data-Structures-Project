...
string doubleOperatorHandler(string input) {
	string y=input;
	while(isDoubleOperatorPresent(y)) {
...
int evaluate(string input)		//Method which will evaluate a infix and return the result
{
	string y = infix2postfix(input);
	stack <int> mystack;
	//2. Scan the postfix expression from left to right for every element
	int index=0;
	bool isCurNumber = true;
	while(index!=(y.length()-1)) {
	string number;
	for (int i = index; i < y.length(); i++){
		if(isdigit(y[i])){
			isCurNumber=true;
			number = number + to_string(y[i]-48);
			index=i;
		} else {
			index=i;
			isCurNumber=false;
	...
	if(number!=""){ 
 	cout <<"PUSHING NUMBER TO STACK: "<< number << " index is now: " << to_string(index) << endl;
	mystack.push(stoi(number)); //maybe no - 48
	number="";
	}

	for (int i = index; i < y.length(); i++){

		//cout<<"NOT A DIGIT OR SPACE" <<endl;
		if((!isdigit(y[i]))&&(y[i]!=' ')) {
	 ...
	 float x1 = mystack.top();
            ...
            if (y[i] == '/') x3 = x2/x1;
            if (y[i] == '^') x3 = pow(x2,x1); 
            cout <<"PUSHING ANS TO STACK: "<< to_string(x3) << endl;
	        mystack.push(x3);
	        index=i;
	        cout<< "INDEX IS NOW: "<<to_string(index)<<endl;
	    }
	    else if (y[i]==' ') {
